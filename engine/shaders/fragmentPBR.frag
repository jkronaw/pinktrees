...
// lights
const vec3 lightPositions[4] = { vec3(2, 2, 5), vec3(-2,2,5), vec3(2,-2,5), vec3(-2,-2,5) };
const vec3 lightColors[4] = { vec3(5), vec3(5), vec3(5), vec3(5) };
...
{		
    ...
    float roughness = texture(texRoughness, exTexcoord).r;
    float metallic = texture(texMetallic, exTexcoord).r;
    float ao = 0.1; // texture(texAO, exTexcoord).r;
